...
/*	TRACE("  %d == %d \n",m_arColOrder[0],pLayer[2]);
	...
	VERIFY(m_arColOrder[0]==pLayer[2]);
	VERIFY(m_arColOrder[1]==pLayer[3]);
	VERIFY(m_arColOrder[2]==pLayer[4]);
	VERIFY(m_arColWidths[0]==pLayer[2+3]);
	VERIFY(m_arColWidths[1]==pLayer[3+3]);
	VERIFY(m_arColWidths[2]==pLayer[4+3]);
	VERIFY(GetColumnCount()==3);
...
LRESULT CGridCtrl::OnTimer(WPARAM wParam, LPARAM lParam)
{
	VERIFY(wParam == WM_LBUTTONDOWN);
...
void CGridCtrl::ResetScrollBars()
{
    ...
    VERIFY(m_nVScrollMax < INT_MAX && m_nHScrollMax < INT_MAX); // This should be fine
...
int CGridCtrl::InsertColumn(LPCTSTR strHeading,
                            ...
                            int nColumn  /* = -1 */)
{
	VERIFY(!m_AllowReorderColumn); // function not implemented in case of m_AllowReorderColumn option
...
{
	VERIFY(Nb == -1 || Nb ==1); // only these vlaues are implemented now
...
{
    VERIFY(nRow >= 0 && nRow < m_nRows && height >= 0);
...
{
    VERIFY(nCol >= 0 && nCol < m_nCols && width >= 0);
                               ...
                               BOOL bResetScroll /*=TRUE*/)
{
    ...
    VERIFY(GVS_DEFAULT <= nAutoSizeStyle && nAutoSizeStyle <= GVS_BOTH);
...
void CGridCtrl::Reorder(int From, int To)
{
	...
	VERIFY(From>= GetFixedRowCount() && To>=GetFixedRowCount()-1 && From<m_nRows && To<m_nRows);
