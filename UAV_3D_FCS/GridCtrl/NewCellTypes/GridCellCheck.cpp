...
* 
* 文件名称：CHUD.cpp
* 摘    要：本项目是为一款基于MFC的添加用户窗口
* 
* 当前版本：1.0
* 修改作者：郑迪威
* 完成日期：2015年1月23日
*
修改摘要：为Check型控件添加左、中、右对齐三个函数
...
//自定义函数 ////////////////////////////////////////////////
void CGridCellCheck::SetFormat(DWORD Format)
{
	m_Format = Format;
}

void CGridCellCheck::SetCellLeft(CRect &place)
{
	int nWidth = GetSystemMetrics(SM_CXHSCROLL);
	place = m_Rect + CSize(GetMargin(), GetMargin());
	place.right = place.left + nWidth;
	place.bottom = place.top + nWidth;
}

void CGridCellCheck::SetCellCenter(CRect &place)
{
	int nWidth = GetSystemMetrics(SM_CXHSCROLL);
	place = m_Rect + CSize(GetMargin() + m_Rect.Width() / 2 - nWidth / 2, GetMargin());
	place.right = place.left + nWidth;
	place.bottom = place.top + nWidth;
}

void CGridCellCheck::SetRadio(BOOL bRadio)
{
	m_bRadio = TRUE;
}

void CGridCellCheck::SetCellRight(CRect &place)
{
	int nWidth = GetSystemMetrics(SM_CXHSCROLL);
	place = m_Rect + CSize(m_Rect.right - GetMargin() - nWidth, GetMargin());
	place.right = place.left + nWidth;
	place.bottom = place.top + nWidth;
}
/////////////////////////////////////////////////////////////
...
void CGridCellCheck::OnClick(CPoint PointCellRelative)
{
	...
	if (m_bRadio)
	{
		INT rows = 0, index_row = 0;
		CGridCtrl *pGridCtrl = GetGrid();
		CGridCellBase *pGridCheck = NULL;
		rows = pGridCtrl->GetRowCount();
		for (index_row = 1; index_row < rows; index_row++)
		{
			if (index_row == cell.row)
			{
				continue;
			}
			pGridCheck = pGridCtrl->GetCell(index_row, cell.col);
			if (!pGridCheck || !pGridCheck->IsKindOf(RUNTIME_CLASS(CGridCellCheck)))
			{
				continue;
			}
			((CGridCellCheck *)pGridCheck)->SetCheck(FALSE);
		}
	}
    ...
    if (!GetGrid()->IsCellEditable(cell))	
	{
		if (m_bCheckEnable)
		{
			//When the cell is readonly but access to check
			CRect place;
			place = GetCheckPlacement();
			//Judge if the point is in the area of check box
			if (PointCellRelative.x >= place.left 
				&& PointCellRelative.x <= place.right 
				&& PointCellRelative.y >= place.top 
				&& PointCellRelative.y <= place.bottom)
			{
				m_bChecked = !m_bChecked;
				this->SetCheck(m_bChecked);
			}
		}
		//The cell can't check or edit
		return;
